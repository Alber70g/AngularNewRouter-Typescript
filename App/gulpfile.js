...
gulp.task('serve-dev:node', ['dev'], function () {
    ...
    exec('dnx ../../Services/src/SampleService web');
    exec('start http://localhost:9000');
});

gulp.task('serve-dev-noproxy:node', ['dev'], function () {
    watch();
...
gulp.task('compile-tests', function() {
    ...
    var tsResult = gulp.src(config.typescript.tests)
        .pipe(sourcemaps.init())
        .pipe(ts({
            noImplicitAny: true
          }));
...
function startProxy() {
    ...
    http.createServer(function(req, res) {
        process.on('uncaughtException', function (err) {
            // Ignore uncaught socket exceptions
        });
        ...
        {
            case 'api':
                proxy.web(req, res, { target: 'http://localhost:61076' });
                break;
